<?php
/**
 * Adds wp_minimalist_Posts_List_Widget widget.
 */
class wp_minimalist_Posts_List_Widget extends WP_Widget {
    /**
     * Register widget with WordPress.
     */
    public function __construct() {
        parent::__construct(
            'wp_minimalist_posts_list_widget',
            esc_html__( 'TSB : Posts List', 'wp-minimalist' ),
            array( 'description' => __( 'A collection of posts from specific category displayed in list layout.', 'wp-minimalist' ) )
        );
    }
                            ...
                            <div class="post-item post-card -tiny format-standard">
                                <div class="post_thumb_image bmm-post-thumb card__cover">
                                    <?php if( $thumbnail_url ) : ?>
                                        <img src="<?php echo esc_url( $thumbnail_url ); ?>" loading="<?php wp_minimalist_lazy_load_value(); ?>">
                                    <?php endif; ?>
                                </div>
    ...
    function widget_fields() {
        ...
        $categories_options[''] = esc_html__( 'Select category', 'wp-minimalist' );
        ...
        return array(
                array(
                    ...
                    'default'   => esc_html__( 'Posts List', 'wp-minimalist' )
                ),
                array(
                    'name'      => 'posts_category',
                    'type'      => 'select',
                    'title'     => esc_html__( 'Categories', 'wp-minimalist' ),
                    'description'=> esc_html__( 'Choose the category to display list of posts', 'wp-minimalist' ),
                    'options'   => $categories_options
                ),
                array(
                    'name'      => 'posts_count',
                    'type'      => 'number',
                    'title'     => esc_html__( 'Number of posts to show', 'wp-minimalist' ),
                    'default'   => 3
                ),
                array(
                    'name'      => 'posts_cat',
                    'type'      => 'checkbox',
                    'title'     => esc_html__( 'Show post categories', 'wp-minimalist' ),
                    'default'   => false
                ),
                array(
                    'name'      => 'posts_excerpt',
                    'type'      => 'checkbox',
                    'title'     => esc_html__( 'Show post excerpt content', 'wp-minimalist' ),
                    'default'   => false
                ),
            );
    }
...
} // class wp_minimalist_Posts_List_Widget