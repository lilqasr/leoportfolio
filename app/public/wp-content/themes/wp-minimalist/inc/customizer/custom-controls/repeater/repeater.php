 ...
 * @since 1.0.0
 */
class Wp_Minimalist_WP_Custom_Repeater extends \WP_Customize_Control {
    /**
     ...
     * Main function
     * 
     * Wp_Minimalist_WP_Custom_Repeater class name 
     */
    ...
    function enqueue() {
        ...
        wp_enqueue_style( 'wp-minimalist-wp-repeater', get_template_directory_uri() . '/inc/customizer/custom-controls/repeater/repeater.css', array(), WP_MINIMALIST_VERSION, 'all' );
        wp_enqueue_script( 'wp-minimalist-wp-repeater', get_template_directory_uri() . '/inc/customizer/custom-controls/repeater/repeater.js', array( 'jquery' ), WP_MINIMALIST_VERSION, true );
    }
    ...
    public function render_content() {
        ...
        $item_count = 1;
        ?>
        <div class="wp-minimalist-repeater-control">
            <label class="customize-control-title"><?php echo esc_html( $this->label ); ?></label>
            <?php if( $this->description ) { ?>
                <span class="description customize-control-description"><?php echo esc_html( $this->description ); ?></span>
            <?php } ?>
            <div class="wp-minimalist-repeater-control-inner">
                <?php foreach( $control_values as $control_value_key => $control_value ) : ?>
                    <div class="wp-minimalist-repeater-item <?php if( $control_value->item_option === 'show' ) : echo 'visible'; else : echo 'not-visible'; endif; ?>">
    ...
    /**
     * Render control field w.r.t paramter 
     * 
     */
    function render_control( $field_key, $field, $current_item_value ) {
        $type = $field->type;
        $label = isset( $field->label ) ? $field->label : esc_html__( 'Item Label', 'wp-minimalist' );
        ...
        switch( $type ) {
            ...
            case 'checkbox' : ?>
                <div class="single-control checkbox-field">
                    <label class="control--item-label">
                        <input class="repeater-field-value-holder" data-default="<?php echo json_encode( esc_attr( $default_value ) ); ?>" data-key="<?php echo esc_attr( $field_key ); ?>" type="checkbox" <?php checked( esc_attr( $current_item_value->$field_key ), true ); ?>>
            ...
            case 'image' : ?>
                <div class="single-control image-field">
                    ...
                    <div class="image-holder">
                        ...
                        <div class="add-image-trigger <?php if( $current_item_value->$field_key ) echo 'no-trigger'; ?>"><?php echo esc_html__( 'Upload Image', 'wp-minimalist'); ?></div>
                ...
                <div class="single-control fontawesome-icon-picker">
                    ...
                    <div class="icon-holder">
                        ...
                        <div class="search-field" style="display:none;">
                            <input type="text" placeholder="<?php echo esc_attr__( 'Type to search social icon', 'wp-minimalist' ); ?>">
                        </div>
            ...
            case 'textarea' : ?>
                <div class="single-control textarea-field">
                    ...
                    <?php if( $description ) { ?>
                        <span class="control--item-description"><?php echo esc_html( $description ); ?></span>
                    <?php } ?>
                    <textarea class="repeater-field-value-holder" data-default="<?php echo esc_attr( $default_value ); ?>" data-key="<?php echo esc_attr( $field_key ); ?>" rows="5"><?php echo esc_html( $current_item_value->$field_key ); ?></textarea>
                </div>
            <?php
            break;
            case 'url' : ?>
            ...
            break;
            case 'wp-editor' : ?>
                <div class="single-control wp-editor-field">
                    <h2 class="control--item-label"><?php echo esc_html( $label ); ?></h2>
                    <?php if( $description ) { ?>
                        <span class="control--item-description"><?php echo esc_html( $description ); ?></span>
                    <?php } ?>
                    <textarea id="<?php echo esc_attr( wp_generate_uuid4() ); ?>" class="repeater-field-value-holder" data-field="wp-custom-editor" data-key="<?php echo esc_attr( $field_key ); ?>" style="width:100%"><?php echo wp_kses_post( $current_item_value->$field_key ); ?></textarea>
                </div>
            <?php
            break;
